...
function normalizeLead(input) {
  return {
    ...
    privacy_policy_accepted: toBoolean(input.privacy_policy_accepted),
...
function validateLead(input) {
  ...
  if (!lead.personal_data_consent) {
    errors.personal_data_consent = "Для отправки заявки необходимо дать согласие на обработку персональных данных и подтвердить ознакомление с политикой обработки персональных данных";
  }

  if (!lead.privacy_policy_accepted) {
    errors.privacy_policy_accepted = "Для отправки заявки необходимо дать согласие на обработку персональных данных и подтвердить ознакомление с политикой обработки персональных данных";
  }
...
function buildLeadEmail(lead) {
  ...
  return {
    ...
    text: [
      ...
      `Ознакомление с политикой ПДн: да, версия ${lead.privacy_policy_version || PRIVACY_POLICY_VERSION}`
    ].join("\n"),
    html: `
      ...
      <ul>
        ...
        <li><strong>Ознакомление с политикой ПДн:</strong> да, версия ${safe.privacy_policy_version}</li>
      </ul>
    `
  };
}
